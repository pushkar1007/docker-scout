...
`
	...
	if cfg.DataDir != "/data/dockerscout" {
		t.Errorf("DataDir = %q, want %q", cfg.DataDir, "/data/dockerscout")
	}
...
func TestDeployGeneratedConfig_LoadsCorrectly(t *testing.T) {
	...
	deployGeneratedConfig := `# dockerscout Agent Configuration (auto-generated)
gateway_url: "nats://10.0.0.1:4222"
...
func TestDeployGeneratedConfig_NoTLS(t *testing.T) {
	...
	deployGeneratedConfigNoTLS := `# dockerscout Agent Configuration (auto-generated)
gateway_url: "nats://10.0.0.1:4222"
token: "deploy-token-xyz"
docker_host: "unix:///var/run/docker.sock"
hostname: "10.0.0.50"
...
func TestEnvOrDefault(t *testing.T) {
	// Without env var set
	result := envOrDefault("DOCKERSCOUT_TEST_NONEXISTENT_12345", "fallback")
	if result != "fallback" {
		t.Errorf("envOrDefault() = %q, want %q", result, "fallback")
	}

	// With env var set
	t.Setenv("DOCKERSCOUT_TEST_ENV_VAR", "custom-value")
	result = envOrDefault("DOCKERSCOUT_TEST_ENV_VAR", "fallback")
